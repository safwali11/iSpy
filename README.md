...
This Python code contains the functions created and used in the project. The functions
are split into categories with each sub-bullet describing the specific functions created
and listed below:
...
* Real-time function (in terminal test display, for testing in Jupyter Notebook and with UART)
	* Real-time image label prediction and output "Look Out"
	* Define the optical flow preliminary values
	* Real-Time calcualtion of sliding window optical flow display images inline Jupyter Notebook
	* Save the raw frames, sliding window optical flow images, and predicitons list
	* Real-Time calcualtion of sliding window optical flow display images in window
	* UART inialization
...
and split into frames. The videos were mainly footage of first person view drones
crashing into trees. The videos were split into crash and no crash frames.
